...
exports.resolvers = {
    Query: {
        getShipments: (_, args, context) => {
            ...
            if (args.search) {
                ...
                result = result.filter(s => s.name.toLowerCase().includes(searchStr) ||
                    s.id.toLowerCase().includes(searchStr) ||
                    s.class.toLowerCase().includes(searchStr) ||
                    s.subjects.some((subj) => subj.toLowerCase().includes(searchStr)));
            }
            // 3. Sorting
            if (args.sortBy) {
    ...
    Mutation: {
        login: (_, args) => {
            const token = jsonwebtoken_1.default.sign({ role: args.role }, SECRET_KEY, { expiresIn: '1h' });
            return { token, role: args.role };
        },
        createShipment: (_, args, context) => {
